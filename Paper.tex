...
% Title.
% ------
\title{CORRECTING DOMAIN-TERM ASR ERRORS BY ACOUSTIC RETRIEVAL WITH A PHONETIC CONSTRAINT}
%
% Single address.
% ---------------
\name{Names\thanks{Thanks to XYZ agency for funding.}}
\address{Korea AI Lab (K-AI Lab), South Koea}
%
% For example:
% ------------
%\address{School\\
...
the recogniser's own pseudo-labels with synthetic speech; the correction
decision itself uses no reference transcript. On Korean clinical and finance speech the method lowers domain-term
error by 25.4\,\% and 16.5\,\% relative under Qwen3-ASR, and by 14.3\,\% and
10.7\,\% under Whisper large-v3-turbo, with overall CER also falling in all four
cases. On four Mandarin domains it lowers domain-term error by 20.1--29.5\,\%
relative. An ablation attributes most of the reduction to the phonetic condition,
the acoustic margin contributing little once it is applied.
\end{abstract}
...
\item \textbf{An exemplar-based framework for post-recognition domain-term
      ...
      recognisers, the framework achieves relative reductions of
      10.7--25.4\,\% in missed reference-term instances, with CER remaining below
      the corresponding baseline at the selected operating points
...
the query spans. In the Korean experiments, carrier sentences are selected from
reference transcripts, restricted to contexts in which the ASR hypothesis
correctly contains the target term.
...
Gallery membership is filtered separately for each query. In the Korean Whisper
experiments, incumbent exemplars from the query utterance are excluded, while
mined term exemplars sharing the query's speaker, utterance, or
recording-session identifier are excluded where those identifiers are available.
The Mandarin experiments additionally exclude same-speaker incumbent exemplars.
For the Korean clinical corpus, source-folder identifiers serve as speaker
proxies and do not guarantee speaker separation.
...
\label{eq:dph}
\end{equation}
where $\operatorname{Lev}$ is Levenshtein distance and $\phi$ maps text to a
language-specific symbol sequence. In the Korean Whisper experiments, $\phi$
decomposes written Hangul syllables into jamo without applying
pronunciation-conversion rules. For Mandarin, $\phi$ uses pinyin initials and
finals with tone information.
...
We evaluate the method on two Korean corpora and four Mandarin domain subsets.
\corpA{} contains 59{,}054 utterances from the D61 split of AI-Hub
\emph{Counselling Speech}, covering financial call-centre conversations.
\corpB{} contains 51{,}898 utterances from the validation split of AI-Hub
\emph{Medical Staff and Patient Speech}. The corresponding evaluation pools
contain 34{,}500 and 34{,}334 utterances. \corpA{} provides 493 speaker
identifiers. \corpB{} provides no speaker identifiers, so its 81 source folders
are used for exemplar exclusion and grouped evaluation; these folders do not
guarantee speaker separation.

The Mandarin data are drawn from AISHELL-1~\cite{bu2017aishell} and divided into
finance (\zhfin{}), legal (\zhleg{}), sport (\zhspo{}) and technology
(\zhtec{}) subsets, containing 52{,}840, 30{,}301, 24{,}028 and 19{,}765
utterances respectively. Since AISHELL-1 does not provide domain annotations,
labels are inferred from reference transcripts in blocks of ten consecutive
utterances. Agreement with independently available prompt-index groupings is
97\,\%, 83\,\%, 92\,\% and 70\,\% for the four domains. Each domain is evaluated
separately.

Each target vocabulary contains 100 terms. Korean candidates must occur in at
least 20 utterances, have a recognition error rate of at least 8\,\%, and appear
across at least five speaker or source groups; candidates are ranked by error
count and screened for domain relevance using Qwen3-4B-Instruct. Mandarin
candidates are ranked by error rate without LLM screening, under thresholds of
15 occurrences, a 10\,\% error rate and five speakers, relaxed for \zhtec{} to
10 occurrences, an 8\,\% error rate and four speakers. Vocabulary selection uses
reference transcripts and the corresponding recogniser's output; term lists and
instance counts therefore differ between recognisers.

We additionally explored domain-term candidate extraction from Korean ASR
pseudo-labels without consulting reference transcripts during candidate
generation. Tokens were analysed with a Korean morphological
analyser~\cite{lee2024kiwi} and filtered to remove grammatical and generic
items, after which the 1{,}000 most frequent surviving candidates were retained;
this pool covers 67 of the 100 curated \corpA{} terms and 64 of the 100 curated
\corpB{} terms. Domain relevance was assessed manually in this exploratory
experiment, while the curated vocabularies were used only to examine coverage
and to guide filter refinement. The resulting pool could subsequently be
screened by an LLM, although that step was not evaluated. This procedure is a
preliminary alternative to reference-based vocabulary selection; the correction
results reported here use the reference-selected term lists.

\subsection{Recognition systems and implementation}

Whisper large-v3-turbo~\cite{radford2023robust} is evaluated on both Korean
corpora and all four Mandarin domains. Qwen3-ASR-1.7B~\cite{qwen2026asr} is
additionally evaluated
on the Korean corpora. Each corrected transcript is compared with the original
output of the same recogniser.

The encoder is HuBERT: layer 10 of \texttt{hubert-base-korean} for Korean and
layer 9 of \texttt{chinese-hubert-base} for Mandarin. Frame representations are mean-pooled
into 768-dimensional vectors and $L_2$-normalised. In the Korean Whisper
experiments, indexed spans use Whisper's word timestamps; flagged utterances are
realigned with Qwen3-ForcedAligner-0.6B~\cite{qwen2026asr} to locate query
spans. Indexed spans
include 0.05\,s of padding on each side; spans shorter than 0.08\,s and labels
shorter than two characters are excluded.

The Korean detector uses Gemini-3-Flash with domain-specific prompts, temperature
zero, and batches of 20 utterances. Its utterance-level recall and precision are
73.8\,\% and 35.2\,\% on \corpA{}, and 89.5\,\% and 75.6\,\% on \corpB{}. For
this calculation an utterance is considered erroneous if its hypothesis omits at
least one reference term.

Synthetic exemplars are generated with Qwen3-TTS~\cite{qwen2026tts} using three
voices. The main
Korean experiments combine mined and synthetic exemplars. The Mandarin
experiments evaluate six gallery configurations: synthetic exemplars, synthetic
exemplars with an estimated embedding-offset correction, up to nine mined
exemplars per term, all mined exemplars, and unions of all mined exemplars with
either synthetic configuration.
...
report CER to assess changes to the complete transcript. Korean CER is averaged
over utterances in the evaluation pool; Mandarin changes in CER are averaged
over all utterances within each domain. Fixed, language-specific rules for
numeral and orthographic normalisation are applied before scoring.
...
The main evaluation is in-sample: vocabulary selection, threshold selection and
scoring use the same evaluation pool. We additionally assess threshold selection
by dividing the Korean speaker or source groups into two subsets using random
seed zero, selecting thresholds on one subset and evaluating on the other with
the remaining pipeline unchanged. Confidence intervals for relative DTER
reductions are estimated from 2{,}000 bootstrap resamples of the evaluation
groups.
...
\begin{table}[t]
\centering
\caption{Korean correction results. $N_{\mathrm{ref}}$ is the number of
reference-term instances. $\Delta$CER is in percentage points.}
\label{tab:main}
...
\midrule
\multirow{2}{*}{\corpA{}}
 & Qwen3-ASR & 69{,}429 & \phantom{0}9.71 & \best{\phantom{0}8.10} & $-0.008$ \\
 & Whisper   & 53{,}270 & \phantom{0}6.12 & \best{\phantom{0}5.46} & $-0.003$ \\
\multirow{2}{*}{\corpB{}}
 & Qwen3-ASR & 39{,}586 & \phantom{0}9.32 & \best{\phantom{0}6.95} & $-0.040$ \\
 & Whisper   & 39{,}215 & 14.91 & \best{12.78} & $-0.062$ \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:main} presents the selected operating point for each Korean
corpus and recogniser. Correction reduces DTER in all four settings while
keeping CER below the corresponding baseline. Under Whisper, DTER decreases from
6.12\,\% to 5.46\,\% on \corpA{} and from 14.91\,\% to 12.78\,\% on \corpB{};
under Qwen3-ASR it decreases from 9.71\,\% to 8.10\,\% and from 9.32\,\% to
6.95\,\% respectively.

The CER reductions are small, particularly on \corpA{}. These results
demonstrate improvements at the selected in-sample operating points. Since the
recognisers use different error-selected vocabularies, comparisons should be
made against the baseline within each row.
...
\begin{table}[t]
\centering
\caption{Controls on Korean Whisper outputs. The text-only row uses the matched
candidate-utterance subset described in Sec.~\ref{sec:setup}.}
\label{tab:controls}
...
\midrule
\zhfin{} & 3{,}566 & 51.8 & 38.1 & $+0.092$ & $+0.00124$ \\
\zhleg{} & 2{,}937 & 45.7 & 36.5 & $+0.034$ & $+0.00067$ \\
\zhspo{} & 5{,}456 & 58.5 & 41.3 & $+0.160$ & $+0.00766$ \\
\zhtec{} & 2{,}141 & 20.7 & 16.5 & $+0.070$ & $+0.00139$ \\
\bottomrule
\end{tabular}
\end{table}
...
Across the six galleries and threshold combinations, none of the 480 evaluated
configurations satisfies the CER constraint. Nevertheless the lowest-DTER
configurations reduce term error in all four domains: DTER decreases from
51.8\,\% to 38.1\,\% in finance and from 58.5\,\% to 41.3\,\% in sport, at CER
increases of 0.092 and 0.160 percentage points.
...
The qualitative examples illustrate that a recognised word can be linguistically
valid while being inappropriate for the utterance: one \corpB{} example replaces
\emph{toyoil} (``Saturday'') with \emph{tohyeol} (``haematemesis''). Such cases
motivate comparing the recognised form with both contextual and acoustic
evidence.

Replacement length also varies. The replacement is shorter than the flagged
string in 363 of 1{,}152 accepted \corpB{} substitutions and 21 of 821 on
\corpA{}. These counts describe changes in string length rather than verified
grammatical-suffix deletion; a separate morphological analysis would be required
to determine how often correction alters grammatical structure.
